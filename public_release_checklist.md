...
- [x] Decide default model mode for public users: `large-v3-turbo`, `large`, or `medium`.
  - Status: Done. Whisper Large is the default/recommended mode; Large Turbo is optional when supported; Medium remains a faster rough-check option.
...
- [x] Confirm no API keys, tokens, local usernames, or machine-specific secrets are committed.
  - Status: Done for README/run scripts. No `/Users/bladeruuner/...` paths remain in public docs or launch scripts.
...
- [x] Keep Windows Speech optional and Windows-only.
  - Status: Done. `windows_speech` is available only on Windows and documented as experimental.
- [x] Keep `both` mode available for comparison.
- [x] Confirm `large-v3-turbo` is optional and gracefully unavailable if the installed Whisper package does not support it.
  - Status: Done. Large Turbo is optional in UI/docs; model load failures now return a clearer user-facing error.
...
- [ ] Benchmark Whisper-only, Apple-only, and both-provider mode.
  - Status: Deferred. Requires repeated local hardware timing runs.
- [ ] Benchmark Windows Speech and Windows `both` mode.
  - Status: Not done. Windows implementation has not been smoke-tested on Windows hardware.
...
- [ ] Fresh clone setup succeeds on a clean macOS machine.
  - Status: Manual release gate. Not completed in this working copy.
- [ ] First run succeeds after installing documented dependencies.
  - Status: Not fully verified from a fresh clone; local tests/build pass.
